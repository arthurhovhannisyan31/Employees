...
class EmployeesList {
  ...
  init() {
    // start module, defines all the all the usages
    app.use(express.json())
    app.use(express.urlencoded({ extended: true }))
    app.engine('hbs', consolidate.handlebars)
    app.set('view engine', 'hbs') // todo remove
        ...
        (err, user, info, next) => {
          if (err) {
            ...
            const token = jwt.sign(
              ...
              `${this.secret}`
            ) //добавить SSH ключ, время жизни токена
  ...
  checkAuthentication() {
    //routes protector, checks if request is authorized
    const mustBeAuthenticated = (req, res, next) => {
      //check for grant access
  ...
  requestHandler() {
    ...
    app.get('/employees', async (req, res) => {
      //send main page
      res.sendFile('employeesList.html', {
        root: path.resolve(__dirname, './public'),
      })
    })
